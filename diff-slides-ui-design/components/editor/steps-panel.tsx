...
export function StepsPanel() {
  const { steps, currentStepIndex, setCurrentStepIndex, addStep, deleteStep, updateStepTitle } = useEditor()
  ...
  const getTotalChanges = () => {
    return steps.reduce((acc, step) => {
      const changes = step.code.filter((line) => line.type !== "unchanged").length
      return acc + changes
    }, 0)
  }
    ...
    <div className="w-80 bg-card border-r border-border flex flex-col">
      <div className="p-4 border-b border-border">
        <div className="flex items-center justify-between mb-4">
          ...
          <Button size="sm" variant="outline" className="h-8 gap-2 bg-transparent" onClick={addStep}>
        ...
        <div className="p-2 space-y-1">
          {steps.map((step, idx) => {
            ...
            return (
              ...
              >
                <button onClick={() => handleSelectStep(idx)} className="w-full text-left">
                  <div className="flex items-start gap-2">
                    <GripVertical className="w-4 h-4 text-muted-foreground mt-1 opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0" />
                    <div className="flex-1 min-w-0">
                      ...
                      {editingId === step.id ? (
                        ...
                        <p
                          ...
                          onDoubleClick={() => handleStartEdit(step.id, step.title)}
                        >
                          {step.title}
                        </p>
                      )}
